...
import { guard } from "../core/guard"; import { limits } from "../utils/limit"; import { isWhitelisted } from "../utils/whitelist";
export default {name:Events.MessageCreate,once:false,async execute(m:any){
 ...
 const author=await m.guild.members.fetch(m.author.id).catch(()=>null);
 if(!author||isWhitelisted(author))return;
 if(/@(everyone|here)\b/i.test(m.content)){
  await m.delete().catch(()=>null);
  const hit=limits.hit(`mention:${m.guild.id}:${m.author.id}`,10000,3);
  if(hit.exceeded)await author.timeout(600000,"Guard: everyone/here spam").catch(()=>null);
